...
class Controller6dof:
    ...
    def trajectoryFollower(self, trajectory, encoderVals, time = None):
        ...
        if self.calledAtRegInterval == True or time == 0 or time - self.prevTime >= self.T:
            ...
            for j in range(self.numJoints):
                # Extract References
                if self.trajIndex <= trajectory.totalTime*trajectory.pps:
                    xref = trajectory.xref[self.trajIndex,j], 
                    vref = trajectory.xref[self.trajIndex,self.numJoints+j]
                    uref = trajectory.uref[self.trajIndex,j]
                else:
                    xref = trajectory.xref[self.trajIndex-1,j]
                    vref = 0
                    uref = 0
                # Command Torque
